-- This pane displays an aggregate (the original) histogram of judgment offsets
-- as well as the mean timing error, median, and mode of those offsets.

local args = ...
local player, _, computedData = unpack(args)
...
if next(offsets) ~= nil then
    ...
    if computedData and computedData.HistogramOriginal then
        histogram = computedData.HistogramOriginal
    else
        histogram = LoadActor("./Calculations.lua",
            {
                offsets = offsets,
        ...
        if computedData then computedData.HistogramOriginal = histogram end
    end
...
pane[#pane + 1] = Def.BitmapText {
    Font = "Common Normal",
    Text = ScreenString("MeanTimingError2"),
    InitCommand = function(self)
        self:x(40):y(label.y)
